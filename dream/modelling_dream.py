    ...
    def forward(
        ...
        if past_key_value is not None:
            if dual_cache:
                ...
                # Fix: Extract indices per batch correctly
                # replace_position shape: [batch, full_seq_len]
                # We need to get position indices, not flattened indices across all batches
                
                # Get the mask for first batch (all batches have same replace pattern)
                replace_mask = replace_position[0]  # Shape: [full_seq_len]
                replace_indices = replace_mask.nonzero(as_tuple=False).squeeze(-1)  # Shape: [num_positions]
                
                # Verify dimensions
                assert replace_indices.shape[0] == q_len, \
                    f"Shape mismatch: replace_indices has {replace_indices.shape[0]} but q_len is {q_len}"
                
                # Create proper batch-aware indexing
                # batch_indices: [[0, 0, ..., 0], [1, 1, ..., 1], ...] shape [bsz, q_len]
                batch_indices = torch.arange(bsz, device=key_states.device).unsqueeze(1).expand(bsz, q_len)
                # pos_indices: [[idx0, idx1, ..., idxN], [idx0, idx1, ..., idxN], ...] shape [bsz, q_len]
                pos_indices = replace_indices.unsqueeze(0).expand(bsz, q_len)
                
                # Update cache with correct indexing
                past_key[batch_indices, pos_indices] = key_states
                past_value[batch_indices, pos_indices] = value_states
